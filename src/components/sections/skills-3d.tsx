  ...
  return (
    <section ref={container} className="min-h-screen flex flex-col justify-center px-4 md:px-20 py-20 relative z-10">
      <div className="max-w-6xl mx-auto w-full">
        <h2 
          ref={headline}
          className="text-4xl md:text-7xl font-black mb-20 text-center text-slate-900"
        >
        ...
        <div ref={categoriesRef} className="space-y-16">
          {techCategories.map((category) => (
            <div key={category.title} className="bg-white/60 backdrop-blur-xl border border-white/40 rounded-3xl p-8 md:p-12 shadow-[0_8px_32px_0_rgba(31,38,135,0.05)]">
              <h3 className="text-2xl md:text-3xl font-bold mb-8 text-slate-800 border-b border-slate-200/60 pb-4">
                {category.title}
              </h3>
              <div className="flex flex-wrap gap-8 justify-center md:justify-start">
                {category.skills.map((skill) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-slate-600 transition-colors duration-300 group-hover:text-blue-500">
                      {skill.name}
                    </span>
                  </div>
                ))}
